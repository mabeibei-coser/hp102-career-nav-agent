...
- 2026-09-25 纯对话开场 Phase 1–2：按用户确认的 A 方案取消新建档案卡和 profile 补卡；更新欢迎语、提示词及 mock。新增无卡连续聊天、明确开始、历史卡恢复、老用户预填和长对话上下文测试。首轮新增测试 5 项如预期失败；实现后相关行为通过。全量单测首轮 146/147，剩余为旧消息数量断言，已更新待复验；首次并发提示词测试曾超过 5 秒，限制 2 worker 后通过。静态方案见 .planning/2026-09-25-chat-opening.md。

- 2026-09-25 纯对话开场 Phase 3 进行中：147 单测通过；typecheck 通过；lint 0 error、19 条既有 unused 警告。桌面 1280×900 与移动端 390×844 截图检查无卡片、无横向溢出。首轮 E2E 发现测试上传入口仍假定有初始卡片，已改为显式开始；重测定位同时匹配历史题目/登录提示，按当前可操作控件和历史保留规则修正。WebKit 2359 缺失，已从官方 CDN 安装，保留旧浏览器文件，待补跑。

- 2026-09-25 纯对话开场完成：新对话仅欢迎语与输入框，普通求职问答不出卡；明确“开始测评 / 填写档案 / 生成职业导航报告”才展示档案卡，历史流程与重新测评保持可用。验证：vitest 147/147；专项登录 6/6（三端）；相关 E2E 71/72 后将唯一失败确认成上传测试过早读取，改为等待 `/api/resume` 完成并精确定位文件名，三端档案卡复验 9/9；Desktop / iPhone 14 / Pixel 7 的核心开场、自由问答、完整报告、刷新、重开均通过。typecheck 通过；lint 0 error、19 条既有 unused 警告；生产 build 通过（保留既有动态 DB 路径追踪 warning）。全程 mock，未调用真实模型，未部署、未推送。
- 2026-09-25 对话发送即时反馈修复：发送时先将用户消息乐观加入对话区并立即清空输入框，“正在思考”显示在其下方；接口成功后用正式消息替换临时消息，失败时撤回临时消息并恢复输入。验证：三端聊天 E2E 12/12，Pixel 7 慢接口用例额外连续 3/3；vitest 147/147；typecheck 通过；lint 0 error、19 条既有 warning。
- 2026-09-25 产品显示名称调整：页面标题、顶部栏、欢迎语和助手角色统一为“就业服务智能体”；仓库名、部署路径和“职业导航报告”等功能名称保持不变。验证：三端聊天 E2E 12/12；vitest 147/147；typecheck、生产 build 通过；lint 0 error、19 条既有 warning。
